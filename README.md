...
- **即時尋路點亮**：由頂部天線節點出發，**劃 Dah (—) 向左分岔、點 Dit (·) 向右分岔**，動態高亮當前字元與走線路徑。
- **支援標準 26 字母、0~9 數字及更正符號**：底部數字晶片動態點亮，完整覆蓋業餘無線電基礎字元。
...
- **📻 半自動機械震報鍵 (Bug Key / Vibroplex)**：
  - 點 Dit 撥片：機械共振式自動高頻連發。
  - 劃 Dah 撥片：全手動控制長音時長，完美還原經典機械式發報手感。

### 3. 🎯 三大實戰練習與挑戰模式
- **⚡ 自由練習模式 (Free Input Mode)**：隨打隨解碼，內建 4 組引導式情境（E、T、SOS 遇險信號、門檻邊界測試）與**國際摩斯筆誤更正訊號 `<HH>` (8 連續點音，自動作廢並刪除上一單字)**。
- **📖 報文挑戰模式 (Text Mode)**：自由輸入報文或選擇經典通聯短句（`SOS`、`CQ CQ DE`、`PARIS` 等），支援**自動示範發報 (Auto Play)**、點劃提示、嚴格模式、打鍵即時比對（綠對紅錯）與發報成績結算卡。
- **🎯 國際科赫闖關 (Koch Method Drill)**：
  - 國際標準 35 關階梯式學習（由 `K` 與 `M` 開始逐步加入新字元）。
  - **防連出保護演算法 (Anti-Streak Protection)**：避免連續 3 題出現同字元。
  - **考核模式切換**：
    - 🟢 **基礎練習模式 (Quick Mode)**：固定字數目標（12 字 / 24 字），自由掌握打鍵節奏。
    - 🏆 **正規考核模式 (Standard Mode)**：限時滾動考核（60 秒 ~ 300 秒），雙排滾動平滑替換。
    - 🔥 **極限挑戰模式 (Challenge Mode)**：高壓限時倒數考核，正確率跌破 90% 立即重置計時，直至達成報務員反射神經標竿！

### 4. 📻 專業無線電聲學與節奏儀表
- **法恩斯沃斯節奏 (Farnsworth Timing)**：音元本身維持高 WPM（如 18~24 WPM）字元速度，僅拉長字元與單字停頓間隔，徹底破除初學者「數點劃」的心魔。
- **天電背景底噪 QRN**：原生 Web Audio 生成粉紅雜訊，模擬短波接收機的真實大氣天電干擾環境。
- **發報節奏與時序儀 (Settlement Gap Timeline)**：
  - $0 \sim 3T$：點劃接續區（金色光條，即時倒數距字元結算毫秒數）
  - $3T \sim 7T$：字元已結算區（青藍光條，字元已確認）
  - $\ge 7T$：單字結算區（翠綠光條，自動插入空格）
