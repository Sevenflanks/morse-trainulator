...
> 純前端、零依賴的摩斯密碼電報模擬器與練習工具。  
> 支援直鍵、雙撥片夾發 (Iambic Mode A/B) 與半自動震報鍵 (Bug Key)，協助練習者循序漸進建立打鍵與聽碼手感。
...
👉 **[線上體驗 Demo 連結](https://sevenflanks.github.io/morse-trainulator/)**  
...
- **電路板視覺風格**：高對比黑金工控面板，焊盤與走線路徑清晰。
- **二元樹尋路**：由頂部天線出發，**劃 Dah (—) 向左分岔、點 Dit (·) 向右分岔**，高亮當前字元與走線路徑。
- **支援 26 字母、0~9 數字及更正符號**：底部數字晶片點亮，完整覆蓋基礎字元。

### 2. ⚡ 三種電鍵裝置模擬 (Three Keyer Modes)
- **🔘 直鍵模式 (Straight Key)**：手動按鍵練習，支援鍵盤、滑鼠點擊與觸控。
- **🎛️ 雙撥片電鍵 (Iambic Paddle Keyer)**：
  - 實作 **Curtis 8044** 夾發邏輯。
  - **點劃記憶 (Element Memory)**：發報中拍擊另一撥片自動排入下一音元。
  - **Mode B（自動補音）** 與 **Mode A（放開即停）** 自由切換。
  - **左右撥片互換**：支援反轉為左劃 Dah / 右點 Dit，適配左手或個人習慣。
- **📻 半自動機械震報鍵 (Bug Key / Vibroplex)**：
  - 點 Dit 撥片：自動連發。
  - 劃 Dah 撥片：手動控制長音長度。

### 3. 🎯 三大練習與考核模式
- **⚡ 自由發報 (Free Mode)**：隨打隨解碼，內建 5 組引導範例與**筆誤更正碼 `<HH>` (8 連續點音，自動刪除上一字元)**。
- **📖 文章模式 (Text Mode)**：輸入自訂文章或選擇範例短句（`SOS`、`CQ CQ DE` 等），支援**示範播放**、點劃提示、嚴格模式與成績統計。
- **🎯 國際科赫闖關 (Koch Method Drill)**：
  - 35 關階梯式學習（由 `K` 與 `M` 開始逐步加入新字元）。
  - **防連出保護演算法**：避免連續 3 題出現同字元。
  - **模式選擇**：
    - 🟢 **基礎練習模式 (Quick Mode)**：固定字數目標（12 字 / 24 字），自由掌握節奏。
    - 🏆 **正規考核模式 (Standard Mode)**：限時滾動考核（60 秒 ~ 300 秒），雙排滾動平滑替換。
    - 🔥 **極限挑戰模式 (Challenge Mode)**：限時考核，正確率跌破 90% 重置計時。

### 4. 📻 聲學與儀表
- **法恩斯沃斯節奏 (Farnsworth Timing)**：字元維持設定速度，僅拉長字元與單字間隔。
- **電台背景雜音 QRN**：生成粉紅雜訊，模擬短波接收機大氣雜音環境。
- **發報與間隔儀表**：
  - **即時 WPM 速度顯示**：遵循國際 PARIS 標準（50 單元/詞），支援單音瞬時測速與滑動滾動平均 WPM 計算，即時反饋手鍵與撥片敲擊速度。
  - $0 \sim 3T$：點劃接續區
  - $3T \sim 7T$：字元已結算區
  - $\ge 7T$：單字間隔區（自動插入空格）
...
### 5. 🖥️ 佈局與設定保存
- **雙欄專注** vs **三欄工作台** 一鍵切換。
- **設定自動保存**：電鍵模式、雙撥片設定、參數與鍵位，重新載入自動復原。
...
│   │   ├── text-mode.js     # 文章模式與示範播放
│   │   └── koch-mode.js     # 科赫闖關雙排滾動與考核計時
│   └── app.js               # 應用程式總調度與開機引導
├── test/                    # 自動化單元測試套件
│   ├── run-all.js           # 測試總執行器 (24 套測試)
│   └── test_*.js            # 各模組獨立斷言測試
